...
$installers = @{
	...
	Ruby = "rubyinstaller-2.0.0-p0-x64.exe";
	WIX = "wix37.exe";
}
...
$vm.RemoteSession({ param($session)
	$session.Execute({ param($context, $sqlServiceAccountName, $dataPath, $logPath)
		...
		$this.CmdLine.IsoExecute("Setup.exe", $arguments, 3010)
	},
...
$vm.RemoteSession({ param($session)
	$session.Execute({ param($context, $visualStudioInstallDef)
		...
		($installDef.AdminDeploymentCustomizations.SelectableItemCustomizations.SelectableItemCustomization | ? { $_.Id = "WebTools" }).Selected = "yes"
...
$vm.RemoteSession({ param($session)
	$session.Execute({ param($context, $dataPath)
		$databaseMdf = Get-Item $context.Installers.AdventureWorksDW
...
$vm.RemoteSession({ param($session)
	$session.Execute({ param($context)
		...
		$context.CmdLine.Execute($context.Installers.Ruby, @("/Silent"), 0)
		$context.AddToPath($rubyBin)
...
$vm.RemoteSession({ param($session)
	$session.Execute({ param($context)
		$context.CmdLine.Execute($context.Installers.WIX, @("/Silent"), 0)
	})
})
...
$vm.RemoteSession({ param($session)
	$session.Execute({ param($context, $gitName, $gitEmail) 
		...
		$context.CmdLine.Execute("unzip", @("-d C:\Program Files (x86)\Git\share\vim\vimfiles", "$($context.InstallersDirectory)\$($context.Installers.VimPsSyntax)"), 0)
	},
	@($gitName, $gitEmail))
})
